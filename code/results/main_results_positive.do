...
foreach out in ratio_local1_std sh_local1 sh_nat1 { 
    ...
    *** ROBUSTNESS #2: keep only candidates with positive firm donations in 1993 ***
    preserve
        keep if year==1993
        keep if std_dons_firms > 0        // strictly positive donors only
        keep id_indiv
        tempfile donors
        save `donors'
    restore
    ...
    * fixed effects	
    ...
    keep if temp>1                // candidates running twice
    drop temp
    ...
    byso id_yearparty: gen temp=_N
    keep if temp>=2               // drop singletons
    drop temp
...
esttab indiv_1 indiv_2 indiv_3 using "$main/Table3positive.txt", ///
replace keep(std_dons_firms)  ///
b(3) se lab nomtitles ///
unstack style(tex) lines compress star(* 0.10 ** 0.05 *** 0.01) nonotes ///
mgroups("\shortstack{Local\\index}" "\shortstack{Local\\references}" "\shortstack{National\\references}", ///
...
esttab indiv_1 indiv_2 indiv_3 indiv_4 using "$main/Table4positive.txt", ///
    replace keep(std_dons_firms) ///
    b(3) se lab nomtitles ///
    unstack style(tex) lines compress star(* 0.10 ** 0.05 *** 0.01) nonotes ///
    mgroups("\shortstack{Left-right\\score}" "\shortstack{Extremeness}"  "\shortstack{Originality\\index}"  "\shortstack{National party\\references}", ///
...
esttab m_* using "$main/Table5.txt", ///
...
esttab m1_* using "$main/Table7a.tex", ///
replace keep(std_dons_firms)  ///
b(3) se lab nomtitles ///
unstack style(tex) lines compress star(* 0.10 ** 0.05 *** 0.01) nonotes ///
...
esttab m1_* using "$main/Table7b.tex", ///
replace keep(std_dons_firms)  ///
b(3) se lab nomtitles ///
unstack style(tex) lines compress star(* 0.10 ** 0.05 *** 0.01) nonotes ///
